...
pub use self::libraries::missing::MissingLibraries;
...
pub fn standard_output_eravm(
    ...
    let libraries = Libraries::from(solc_input.settings.libraries.clone());
    let linker_symbols = libraries.as_linker_symbols()?;
...
pub fn link_eravm(paths: Vec<String>, libraries: &[String]) -> anyhow::Result<()> {
    ...
    let linker_symbols = Libraries::try_from(libraries)?.as_linker_symbols()?;
